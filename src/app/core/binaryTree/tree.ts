...
const real = (): Node => {
  ...
  const root = new Node('root', { todo: '', place: 'place' });
  // height 1
  root.left = new Node('실내', { todo: '말씀 읽기를 하세요.', place: '본당' });
  root.right = new Node('실외', { todo: '피구를 하세요.', place: 'Gym' });
  // height 2
  ...
  // height 3
  root.left.left.left = new Node('좌우', { todo: 'Archery를 하세요.', place: '로프 코스 옆' });
  root.left.left.right = new Node('상하', { todo: '단체 줄넘기를 하세요.', place: 'Gym' });
  root.left.right.left = new Node('혼자 있는 것', {
    todo: '헤쳐있다가 고깔모자 쓰고 다시 모이기를 하세요.',
    place: 'Gym'
  });
  root.left.right.right = new Node('그룹과 같이 있는 것', { todo: '로프 코스를 하세요.', place: '로프 코스' });
  root.right.right.left = new Node('좌우', { todo: 'Archery를 하세요.', place: '로프 코스 옆' });
  ...
  root.left.right.left.right = new Node('요나', {
  ...
  root.right.right.left.left = new Node('요나', {
    todo: '수영장으로 가서 고깔모자를 쓰고 물총으로 수영장 위에 있는 물건들을 맞추기를 하세요.',
    place: '수영장'
  });
  ...
  root.right.right.right.right = new Node('상하', { todo: '단체 줄넘기를 하세요.', place: 'Gym' });
  // height 5
